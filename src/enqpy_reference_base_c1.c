...
/* =============================================================================
 * Enqpy(tm) Stream Cipher -- C Reference Implementation  Rev 2.0  [BASE CIPHER]
 * Copyright (c) 2026 NQP LLC (Paul McGough).
 *
 * WHAT THIS FILE IS
 * -----------------
 * This is the canonical software reference for the Enqpy(tm) BASE CIPHER --
 * the proof-complete profile on which the Rev 2.0 formal results are proved
 * CLOSED on both axes:
 *
 ...
 *
 * The Base Cipher is the Ideal Enqpy Configuration (nonce-only OffsetKey
 * derivation / Key Role Separation) RESTRICTED to Case-1 W generation, with a
 * normative 2,048-byte (HIGH) plaintext window and synchronized key update.
 * It is deliberately a STRICT SUBSET of the Extended Mixing Profile (the
 * three-case CS-ordered cipher in enqpy_reference.c): there is no Case
 * Selector, no Phase 2B, and W is generated by Case 1 alone. That restriction
 * is exactly what makes the (EK,QK) -> W map a Z16-module homomorphism, which
 * is what closes the message-axis min-entropy theorem (Theorem 3).
 *
 * RELATIONSHIP TO THE EXTENDED REFERENCE (enqpy_reference.c)
 * ---------------------------------------------------------
 * Same Phase 1 (OR_EXP/eff_or), same Phase 2 (nonce-only VKP/OKP; VKC/OKC),
 * same PDAF primitive, same NIL-Comm primitives. The differences are exactly
 * the Base/Extended boundary documented in the FCD Rev 2.0:
 *
 *   - Phase 2B (Case Selector) is OMITTED entirely. No CS, no CS_PERM table.
 *   - Phase 3 (W generation) is Case 1 ONLY:
 *         W[c*n + p] = (OKC[p] + VKC[(p + c) mod n]) mod 16
 *     yielding n^2 nibbles = 2,048 bytes per window at HIGH (vs 3n^2 = 6,144).
 *   - w_byte_max = n^2 / 2 = 2,048 (HIGH), the normative window bound (R7).
 *   - Phase 5 performs NO CS re-derivation (there is no CS).
 *   - Credential rotation (NIL-Comm) REQUIRES Method 2 (external entropy);
 *     Method 1 is prohibited as the Base-profile rotation mechanism (R7),
 *     because its deterministic chain collapses the equivocation coset 4->1
 *     (see FCD Rev 2.0 7.4 / proof Lemma B4 + Remark). The OWC/Mode-0
 *     primitives are retained; only the policy gate differs.
 *
 * The PUBLIC API (PDAF, PDAF_SEC, ENQPY_NIL_COMM_UPDATE, OWC, enqpy_init) is
 * source-compatible with the Extended reference, so this file is a drop-in
 * object for the AEAD benchmark harness (aead_bench.c): build this file
 * instead of enqpy_reference.c and the harness measures the Base profile.
 *
 * LICENSE
 * -------
 ...
 *
 * ALGORITHM OVERVIEW (Base Cipher)
 * --------------------------------
 ...
 *
 *  Phase 2  VKP, OKP, VKC, OKC  [IDEAL CONFIGURATION -- nonce-only pointers]
 ...
 *
 *  Phase 3  W keystream generation -- CASE 1 ONLY
 ...
 *
 *  Phase 5  Key update (no CS re-derivation)
 ...
 *
 * OFFICIAL TEST VECTORS (Rev 2.0 -- Base Cipher)
 * ---------------------------------------------
 *   PDAF primitive (n=10, 30-nibble output) -- shared with Extended ref:
 *     VK = FB382C001A   OK = CC69100AB4
 *     Mode 0 -> B7913C0ACE7FEBD00B53F4851014AF
 *     Mode 1 -> 7DD02C010CDF74C01B5BF8D811B92B
 *
 *   PDAF_SEC Base Cipher (n=64, 8-byte zero plaintext), canonical keys:
 ...
 *     Base C1 W[0..7]  = 24 34 B5 88 45 C6 FD E8   (CT = W, plaintext zeros)
 *     Base C1 W[0..15] = 24 34 B5 88 45 C6 FD E8 A3 38 55 C3 6B 7D A1 96
 ...
 *
 *   NOTE: These differ from the Extended profile's W (24 C4 3F 3E 94 9B BC 35)
 *   because the Extended W interleaves three case streams; the Base W is the
 *   Case-1 stream alone. Same Phase 1/2 derivation, different Phase 3.
 *
 * BUILD
 * -----
 *   # Library only:
 *   cc -O2 -std=c11 -c enqpy_reference_base_c1.c -o enqpy_base_c1.o
 *   # With self-test:
 *   cc -O3 -std=c11 -DENQPY_SELFTEST enqpy_reference_base_c1.c -o enqpy_base_test
 *   # With benchmark:
 *   cc -O3 -std=c11 -DENQPY_BENCHMARK enqpy_reference_base_c1.c -o enqpy_base_bench
 *   # AEAD bench drop-in (build THIS instead of enqpy_reference.c):
 *   cc -O3 -march=native -std=c11 -c enqpy_reference_base_c1.c -o enqpy_ref.o
 *   cc -O3 -march=native -std=c11 -D_POSIX_C_SOURCE=200809L aead_bench.c enqpy_ref.o -o aead_bench_base
 * ============================================================================= */
...
/* Base Cipher: Case-1 only.  W is n^2 nibbles = n^2/2 bytes per window. */
 ...
 * Used exclusively in the Nil-Communication Key Update path.  Retained from the
 * Extended reference verbatim; the Base Cipher uses it only under Method 2.
 * ============================================================================ */
...
/* ============================================================================
 * SECTION 5 -- W KEYSTREAM GENERATION  [BASE CIPHER: CASE 1 ONLY]
 *
 * W[c*n + p] = (OKC[p] + VKC[(p + c) mod n]) mod 16, for c,p in 0..n-1.
 * This is a single Z16-linear stream (the Case-1 "engine" of the Extended
 * reference, in isolation). n^2 nibbles are packed 2-per-byte, in order, with
 * no interleaving and no running-parity branch (n is even, so n^2 is even and
 * every pair (2k, 2k+1) forms one byte exactly).
 * ============================================================================ */

static void generate_w_c1(const uint8_t *vkc, const uint8_t *okc, int n,
                          uint8_t *w_bytes)
{
    uint8_t vk_t[ENQPY_TILE_LEN];
    uint8_t ok_t[ENQPY_TILE_LEN];
...
/* ============================================================================
 * SECTION 6 -- PDAF_SEC (Main Encrypt / Decrypt)  [BASE CIPHER]
 *
 * Phases 1 and 2 are identical to the Extended reference (nonce-only pointer
 * derivation / Key Role Separation). Phase 3 is Case-1 only; Phase 5 has no
 * CS re-derivation; the window is n^2/2 = 2,048 bytes at HIGH.
 * ============================================================================ */
             ...
             int nTextLen, uint8_t *restrict out)
{
    ...
    /* -----------------------------------------------------------------------
     * PHASE 2 -- VKP, OKP, VKC, OKC  [IDEAL CONFIGURATION -- nonce-only]
    ...
    /* -----------------------------------------------------------------------
     * PHASES 3, 4, 5 -- Streaming loop (Base Cipher: Case-1 W, no CS)
     * Window bound: w_byte_max = n^2/2 (= 2,048 at HIGH).
     * ----------------------------------------------------------------------- */
    uint8_t w_bytes[ENQPY_W_BYTES];
    int w_byte_max = (n * n) / 2;     /* Base Cipher window (R7) */
    int text_done  = 0;

    while (text_done < nTextLen) {
        /* Phase 3 -- Case 1 only */
        generate_w_c1(vkc, okc, n, w_bytes);
        ...
        /* Phase 5 -- Key update (NO CS re-derivation in the Base Cipher)
         *   VKNext = PDAF1(OKC, VKP, Mode 1)
         *   OKNext = PDAF1(VKC, OKP, Mode 1)
        ...
        memcpy(vkc, vkn, n);
        memcpy(okc, okn, n);
        /* No CS re-derivation: the Base Cipher has no Case Selector. */
    }
 ...
 *
 * BASE CIPHER POLICY (Rev 2.0, R7 / proof Lemma B4 + Remark):
 *   Method 2 (external entropy) is REQUIRED for Base-profile credential
 ...
 *   The OWC + Mode-0 seed derivation itself is identical to the Extended ref.
 * ============================================================================ */
                          ...
                          uint8_t *ek_new, uint8_t *qk_new)
{
    ...
    /* Base Cipher: Method 2 is mandatory; Method 1 is prohibited (R7). */
...
/* ============================================================================
 * SECTION 8 -- SELF-TEST HARNESS  (Base Cipher KATs)
 * ============================================================================ */
...
static int selftest(void)
{
    ...
    /* TV3: PDAF_SEC Base Cipher -- W[0..7] = 24 34 B5 88 45 C6 FD E8 */
    {
    ...
    /* TV8: NIL-Comm Method 2 required; Method 1 rejected (Base policy) */
    {
...
/* ============================================================================
 * SECTION 9 -- BENCHMARK HARNESS  (Base Cipher)
 * ============================================================================ */
...
static void bench_w_gen(void)
{
    ...
    for(int w=0;w<BENCH_WARMUP;w++) generate_w_c1(vkc,okc,64,wb);
    double t0=now_ns();
    for(int it=0;it<BENCH_ITERS;it++){generate_w_c1(vkc,okc,64,wb);use_buf(wb,ENQPY_W_BYTES);}
    double el=now_ns()-t0;
...
{
    printf("\n--- PDAF_SEC Base Cipher throughput (n=64) ---\n");
...
int main(void)
{
    printf("Enqpy(tm) Stream Cipher -- Base Cipher Reference  Rev 2.0\n");
    printf("Ideal Configuration, Case-1 W generation (proof-complete profile)\n");
