...
As of 2026, Enqpy™ carries a formal information-theoretic proof of a non-vanishing **ciphertext-only, single-key-epoch** key equivocation — a specific information-theoretic property distinct from computational security, motivated by Shannon's Ideal-System target. The One-Time Pad achieves Perfect Secrecy (a strictly stronger property) but requires a key equal in length to the message, making it impractical for general use. Enqpy™ establishes the ciphertext-only equivocation property — non-vanishing key equivocation under unlimited ciphertext observation within one key epoch — with a fixed, reusable finite key, in the Canonical Enqpy Configuration defined in companion document [12]. It does **not** claim reused-key known-plaintext security for the bare core; that boundary is stated in §8.5 and is addressed in deployment by Enqpy-HKU (§8.10). The recommended default deployment is independent single-window credentials (§8.10.1), which rests on credential independence and invokes no reset lemma; Theorem 4 of [12] addresses the alternative continuing-key-state path and is conditional on the fresh-HKU reset lemma.

This claim is formally established in companion document [12]:

- [12] Enqpy™ Core and Enqpy-HKU — Finite-Key Ciphertext-Only Equivocation and a Fresh-Key-Epoch Deployment Profile (Rev 4.0, 2026). Establishes Lemma 1 (MOD16 fiber size), Theorem 1 (PDAF Mode 1 preimage lower bound |P(O*)| ≥ 2 worst case / ≥ 16 non-degenerate), Theorem 2 (Non-Vanishing Ciphertext-Only Key Equivocation — proved for the Canonical Enqpy Configuration: H(EK,QK|T^∞) = log₂(4) = 2 bits exactly in the ciphertext-only view; exact conditional independence T_{>t} ⊥ (EK, QK) | T_{≤t}), Theorem 3 (Ciphertext-Only Plaintext Equivocation — closed: |S(CT,OR)| ≥ 2^128 for HIGH profile, uniform posterior over the full consistent set, giving H = H∞ ≥ 128 bits), the Section 16 known-plaintext boundary (cross-OR transfer at ≈2 fully-known windows), and Theorem 4 (Enqpy-HKU known-plaintext non-accumulation for the continuing-key-state path, conditional on the fresh-HKU reset lemma) and Proposition 2 (unconditional single-window non-accumulation from credential independence, §8.10.1, invoking no reset lemma). Corollary 2 states the quantum-era posture (ciphertext-only bounds rest on no computational-hardness assumption). Appendix B provides the extended algebraic proofs: exact preimage count formula, equivocation tightness over ℤ₁₆, cycle case completeness, and exact solution count with cycles. The [+8] global shift invariant extends to a full ciphertext-equivalent coset under the key role separation of the Canonical Configuration.
...
- **Enqpy-HKU deployment profile (§8.10) — new.** The normative deployable profile: the core run under NIL-Comm external-entropy key-epoch rotation (before a second fully-known window) with encrypt-then-MAC over eff_or ‖ ciphertext ‖ metadata. Two conforming paths are specified: a **single-window credential rule with independent per-segment keys** (§8.10.1) — the recommended default, which rests on the ciphertext-only theorems alone and invokes no reset lemma — and the continuing-key-state external-entropy rotation, which uses less per-rotation bandwidth but depends on the fresh-HKU reset lemma.
...
| **Document** | **Content** |
| --- | --- |
| [12] Enqpy Core and Enqpy-HKU — Formal Proof, Rev 4.0 | Lemma 1 (MOD16 fiber size = 16, algebraic); Theorem 1 (PDAF Mode 1 preimage lower bound │P(O*)│ ≥ 2 worst case / ≥ 16 non-degenerate); Theorem 2 (Non-Vanishing Ciphertext-Only Key Equivocation — Canonical Configuration: H(EK,QK│T^∞) = log₂(4) = 2 bits exact in the ciphertext-only view; T_{>t} ⊥ (EK, QK) │ T_{≤t}; six-step proof); Theorem 3 (Ciphertext-Only Plaintext Equivocation — closed: │S(CT,OR)│ ≥ 2^128 for HIGH profile, uniform posterior over the full consistent set, H = H∞ ≥ 128 bits); §16 known-plaintext boundary (cross-OR transfer at ≈2 fully-known windows); Theorem 4 (Enqpy-HKU known-plaintext non-accumulation for the continuing-key-state path, conditional on the fresh-HKU reset lemma — load-bearing only for that path) and Proposition 2 (unconditional single-window non-accumulation, §8.10.1, no reset lemma); Corollary 2 (quantum-era posture). Appendix B provides the extended algebraic proofs: exact preimage count formula │P(O*)│ = Σ_{compatible OK} ∏_i f(μ_{i,odd}(OK)) [f(0)=16, f(k≥1)=2]; equivocation tightness (rank-ceiling argument over ℤ₁₆); cycle case completeness; exact solution count with cycles. Ciphertext-only bounds are algebraic; the Enqpy-HKU deployment theorem is conditional on the reset lemma. |
...
## 8.10.1  Single-Window Deployment and Long-Message Segmentation

§8.5 establishes two distinct properties, which require different treatment. The first is *within-window determinability*: a rank-complete set of known plaintext within one 2,048-byte window (e.g. 64 known bytes at HIGH) linearly determines the remaining keystream of **that** window. The second is *cross-window / cross-message propagation*: under one continuing key state, the deterministic Phase 5 chain carries that determination into later windows and, across messages, into later messages.

Within-window determinability is structural and is **not** removed by any key-management discipline. A window that contains a rank-complete known-plaintext set is itself recoverable; this is a permanent property of Case-1 W generation and is retained as a stated limit. The deployment objective is therefore to **confine** an exposure to the single window in which it occurs — never to claim that the exposed window is saved.

**Single-window credential rule (normative).** A given (EK, QK) credential MUST encrypt at most one Enqpy window — at HIGH, at most 2,048 plaintext bytes. The in-session Phase 5 update MUST NOT be used to continue a single message beyond one window under the same credential: the Enqpy window boundary is also the credential boundary.

**Long-message segmentation (normative).** A plaintext longer than one window is segmented at the application layer into segments of at most one window (2,048 bytes at HIGH). Each segment *j* is encrypted as an independent single-window message with its own independently generated credential pair (EK_j, QK_j) (§7.1 CSPRNG requirements, EK_j ≠ QK_j), its own per-message nonce eff_or_j (§7.2), and its own encrypt-then-MAC tag over eff_or_j ‖ CT_j ‖ segment metadata (§8.6), together with ordering and length metadata sufficient for reassembly. The segment credentials MUST be fresh, independent, and used once; they MUST NOT be a deterministic key chain, a counter schedule, or a KDF/PRG expansion from a single seed. Independence is the security-bearing requirement: a deterministic relation between segment credentials would relocate the §8.5 propagation from the keystream layer to the key layer, where the same known-plaintext accumulation applies.

**Proof basis (no reset lemma required).** Under the single-window credential rule with independent per-segment credentials, each segment is exactly the single-key-epoch, single-window object covered by the ciphertext-only results of §8.8 — Theorem 2 (key-axis 2-bit floor) and Theorem 3 (message-axis ≥ 128-bit min-entropy). Cross-segment known-plaintext non-accumulation follows immediately from credential independence: a known-plaintext exposure in one segment constrains only that segment's keys, which are independent of every other segment's. This deployment path therefore does **not** invoke the fresh-HKU reset lemma or Theorem 4 of [12]; those are required only for the continuing-key-state path of §8.10, where one credential spans multiple windows and a genuine reset must be proved. The independent-key segmentation path rests on the original ciphertext-only theorems together with key independence.

**Cost and relationship to continuing-key-state HKU.** Per-window non-accumulation requires fresh secret key material per window, either as independent segment credentials or as a continuing-key-state HKU reset; under this profile the fresh secret is the independent segment credential (≈ 512 bits per 2,048-byte segment at HIGH, distributed out-of-band per §7.1/§7.4). The continuing-key-state path of §8.10 (NIL Method 2, ≈ 128 bits of E_ext per rotation) trades lower per-rotation bandwidth for dependence on the fresh-HKU reset lemma. The two paths are otherwise equivalent in the property they provide — confinement of a known-plaintext exposure to a single window — and in what neither provides: recovery of the already-exposed window.
