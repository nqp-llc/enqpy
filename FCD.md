---
title: "Enqpy™ Stream Cipher — Formal Cryptographic Description"
subtitle: "Complete Specification of the OWC, PDAF, and PDAF_SEC Primitives"
revision: "5.1"
author: "Paul McGough"
publisher: "NQP LLC, Manassas, Virginia"
license: "CC-BY-4.0 (documents); Apache-2.0 (reference code and test vectors)"
source: "Enqpy_FCD_Rev5_1.docx"
generated: "2026-09-13"
note: >
  Markdown rendering of the canonical .docx. The two are the same document;
  where they disagree the .docx governs. This file is the hashed canonical
  artifact listed in PROVENANCE.md.
---

## Contents

- [Document Overview](#document-overview)
  - [Formal Proof Status — Ciphertext-Only Plaintext Equivocation, with a Stated Known-Plaintext Boundary](#formal-proof-status--ciphertext-only-plaintext-equivocation-with-a-stated-known-plaintext-boundary)
  - [Changes in Revision 5.1](#changes-in-revision-51)
- [1. Introduction](#1-introduction)
  - [1.1 Scope](#11-scope)
  - [1.2 Shannon’s Ideal System — Background](#12-shannons-ideal-system--background)
- [2. Notation and Conventions](#2-notation-and-conventions)
- [3. Security Profiles and Minimum Requirements](#3-security-profiles-and-minimum-requirements)
  - [3.1 Overview](#31-overview)
  - [3.2 Security Profile Definitions](#32-security-profile-definitions)
  - [3.3 Prohibited Configurations](#33-prohibited-configurations)
  - [3.4 Key Entropy Requirements](#34-key-entropy-requirements)
  - [3.5 Quantum Security Advisory](#35-quantum-security-advisory)
- [4. Cipher Overview](#4-cipher-overview)
  - [4.1 System Model](#41-system-model)
  - [4.2 Operational Flow Summary](#42-operational-flow-summary)
- [5. Cryptographic Primitives](#5-cryptographic-primitives)
  - [5.1 One-Way Computation (OWC)](#51-one-way-computation-owc)
  - [5.2 Pseudo-random Data Augmentation Function (PDAF)](#52-pseudo-random-data-augmentation-function-pdaf)
- [6. PDAF_SEC Cipher — Complete Specification](#6-pdaf_sec-cipher--complete-specification)
  - [6.1 Overview](#61-overview)
  - [6.2 Parameters](#62-parameters)
  - [6.3 Phase Descriptions](#63-phase-descriptions)
- [7. Key Management](#7-key-management)
  - [7.1 Initial Credential Distribution](#71-initial-credential-distribution)
  - [7.2 Per-Record OR Generation, Counter Persistence, and Credential Retirement](#72-per-record-or-generation-counter-persistence-and-credential-retirement)
  - [7.3 In-Cipher Key Update (Phase 5)](#73-in-cipher-key-update-phase-5)
  - [7.4 Nil-Communication Key Update](#74-nil-communication-key-update)
  - [7.5 Key Size Flexibility](#75-key-size-flexibility)
- [8. Security Analysis](#8-security-analysis)
  - [8.1 Security Claims](#81-security-claims)
  - [8.2 PDAF Mode 1 as the Architectural Key Protection Boundary](#82-pdaf-mode-1-as-the-architectural-key-protection-boundary)
  - [8.3 Perfect Security Cross Analysis](#83-perfect-security-cross-analysis)
  - [8.4 Underdetermination Property — Formal Statement](#84-underdetermination-property--formal-statement)
  - [8.5 Known-Plaintext Boundary](#85-known-plaintext-boundary)
  - [8.6 Integrity and Authentication Guidance](#86-integrity-and-authentication-guidance)
  - [8.7 Comparison with Standard Ciphers](#87-comparison-with-standard-ciphers)
  - [8.8 Formal Proof Status — Ciphertext-Only Equivocation on Two Axes](#88-formal-proof-status--ciphertext-only-equivocation-on-two-axes)
- [8.9 Canonical Enqpy™ Configuration](#89-canonical-enqpy-configuration)
  - [8.9.1 Definition](#891-definition)
  - [8.9.2 Why This Configuration Delivers the Ciphertext-Only Equivocation Property](#892-why-this-configuration-delivers-the-ciphertext-only-equivocation-property)
  - [8.9.3 Operational Requirements](#893-operational-requirements)
  - [8.9.4 Key Role Separation — The Minimal Sufficient Boundary](#894-key-role-separation--the-minimal-sufficient-boundary)
  - [8.10 Deployment Profile — Independent Record Credentials](#810-deployment-profile--independent-record-credentials)
  - [8.10.1 Record Deployment and Internal Windowing](#8101-record-deployment-and-internal-windowing)
  - [8.11 Optional Known-Plaintext Hardening (ESE)](#811-optional-known-plaintext-hardening-ese)
- [9. Singular Capabilities](#9-singular-capabilities)
- [10. Performance and Efficiency](#10-performance-and-efficiency)
  - [10.1 Architecture](#101-architecture)
  - [10.2 Speed](#102-speed)
  - [10.3 Code Size](#103-code-size)
  - [10.4 Bandwidth Overhead](#104-bandwidth-overhead)
- [11. Implementation Products](#11-implementation-products)
- [12. C Reference Implementation](#12-c-reference-implementation)
  - [12.1 Overview](#121-overview)
  - [12.2 Function Signatures (Section 12 — Complete API)](#122-function-signatures-section-12--complete-api)
  - [12.3 Implementation Security Requirements](#123-implementation-security-requirements)
- [13. Test Vectors](#13-test-vectors)
  - [13.1 OWC Test Vector](#131-owc-test-vector)
  - [13.2 PDAF Test Vectors](#132-pdaf-test-vectors)
  - [13.2a PDAF Mode 1 Self-Referential Test Vector (VK = OK — Phase 1 OR_EXP)](#132a-pdaf-mode-1-self-referential-test-vector-vk--ok--phase-1-or_exp)
  - [13.3 PDAF_SEC Round-Trip Test](#133-pdaf_sec-round-trip-test)
- [14. NIST SP 800-22 Randomness Test Results](#14-nist-sp-800-22-randomness-test-results)
  - [14.1 Overview](#141-overview)
  - [14.2 Test Configuration](#142-test-configuration)
  - [14.3 Test Results](#143-test-results)
  - [14.4 Interpretation](#144-interpretation)
- [Acknowledgments](#acknowledgments)
- [Appendix A — Enqpy™ Operational Flow Diagram](#appendix-a--enqpy-operational-flow-diagram)
  - [ENCRYPT](#encrypt)
  - [DECRYPT](#decrypt)
- [Appendix B — Full PDAF Output Test Vectors](#appendix-b--full-pdaf-output-test-vectors)
  - [B.1 PDAF Mode 0 Full n² Output (n = 16, 256 nibbles)](#b1-pdaf-mode-0-full-n²-output-n--16-256-nibbles)
  - [B.2 PDAF Mode 1 Full n² Output (n = 16, 256 nibbles)](#b2-pdaf-mode-1-full-n²-output-n--16-256-nibbles)
- [Appendix C — Security Definitions](#appendix-c--security-definitions)
  - [C.1 Shannon Perfect Secrecy (OTP Form)](#c1-shannon-perfect-secrecy-otp-form)
  - [C.2 Shannon Ideal System](#c2-shannon-ideal-system)
  - [C.3 Underdetermination](#c3-underdetermination)
  - [C.4 PDAF Mode 1 Entangled Underdetermination](#c4-pdaf-mode-1-entangled-underdetermination)
  - [C.5 Computational Security (for comparison)](#c5-computational-security-for-comparison)
- [Appendix D — References](#appendix-d--references)
  - [Formal Proof Documents](#formal-proof-documents)
...
Enqpy™ claims confidentiality via mathematical underdetermination rather than computational hardness. Its primary formal result is a non-vanishing **ciphertext-only plaintext equivocation** (Theorem 3): for any ciphertext and public nonce, at least 2^128 plaintexts (HIGH profile) remain consistent in the stated single-key model, for every plaintext distribution; under the stated uniform key and plaintext priors the posterior is uniform over the full consistent set. A supporting key-axis result identifies the permanent \[+8\] keystream-equivalence coset. Both results are single-key-epoch and ciphertext-only, and are bounded by a stated known-plaintext boundary (§8.5). An optional, experimental within-record hardening layer that raises that threshold for deployments needing it is specified in §8.11; it does not modify the proved results.
...
- \[12\] Enqpy™: Ciphertext-Only Plaintext Equivocation in a Finite-Key MOD16 Stream Cipher, with a Characterized Known-Plaintext Boundary (Rev 5.1, 2026). Establishes Lemma 1 (MOD16 fiber size), Theorem 1 (PDAF Mode 1 preimage lower bound \|P(O\*)\| ≥ 2 worst case / ≥ 16 non-degenerate), **Theorem 3 (Ciphertext-Only Plaintext Equivocation — the primary result: \|S(CT,OR)\| ≥ 2^128 for HIGH for every plaintext distribution; under the stated uniform key and plaintext priors, uniform posterior and H = H∞ ≥ 128 bits)**, Theorem 2 (the permanent four-key \[+8\] keystream-equivalence coset — a ciphertext-only key-equivocation floor of ≥ 2 bits, exact under effective-keystream observation), the Section 16 known-plaintext boundary (cross-OR transfer demonstrated on the published reference instance to be possible with two fully-known windows under a continuing key state; the general threshold is uncharacterized), and Proposition 2 (information-theoretic cross-record non-accumulation under independently sampled per-record keys). Corollary 2 states the quantum-era posture (the ciphertext-only bounds rest on no public-key hardness assumption). Appendix A provides the extended algebraic proofs. The \[+8\] global shift invariant extends to a full ciphertext-equivalent coset under the key role separation of the Canonical Configuration.
...
## Changes in Revision 5.1

Revision 5.1 applies the corrections identified in independent review of Rev 5.0 (see the Rev 5.1 Change Log): the message-axis monotonicity argument is withdrawn, deterministic nonce uniqueness is withdrawn, Non-Reinstantiation is demoted to a non-security-bearing probabilistic statement with its collision family disclosed, the theorem-bearing key-generation hypothesis is raised to full uniform sampling, the AES/ChaCha key-equivocation comparison is withdrawn, message-axis “coset” is corrected to “XOR translate”, the known-plaintext residual counts are labelled as reference-instance measurements, and the deployment rule is stated as two named profiles. No theorem statement or numerical claim was weakened or added. Revision 5.0 completed the realignment and brought the specification into agreement with the Rev 5.0 companion proof and the public V5.0 materials.

- **Primary result is plaintext equivocation.** The headline confidentiality result is now the ciphertext-only **message-axis** theorem (\|S(CT,OR)\| ≥ 2^128; uniform posterior and ≥ 128-bit min-entropy under the stated uniform key and plaintext priors; Theorem 3). The key-axis result is presented as a supporting structure.
...
- **Optional known-plaintext hardening specified (§8.11).** An optional ESE (Enqpy-Scramble-Enqpy) wrapper is specified for deployments that must harden the known-plaintext boundary (§8.5) beyond record-sizing confinement. It is experimental; its guarantee is conditional and computational and belongs to the wrapper; it does not modify the base cipher or the results of §8.8.
...
| **Symbol**  | **Definition**                                                                                                                                                                                                                                           |
|-------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| EK          | Encryption Key — master secret, shared out-of-band. MUST be independently and uniformly generated, independently of QK. No EK ≠ QK restriction applies in the Canonical theorem-bearing profile (§3.3). Minimum length per security profile (Section 3). |
...
KEY SAMPLING (normative). EK and QK MUST be independently and uniformly sampled over the full canonical key spaces from a CSPRNG meeting §7.1. Implementations MUST NOT reject or resample any particular sampled key VALUE — including EK = QK, all-zero, all-same-value or visually patterned values. Entropy is a property of the source and its distribution, not of an individual sampled string, and rejection sampling punctures the key domain over which Theorems 2 and 3 are proved (see the key-domain note in this section). The following configurations remain prohibited under all security profiles: key length n \< 32 nibbles (128 bits); a key source that is not a CSPRNG meeting §7.1, or that does not deliver full-entropy independent uniform output; OR reuse with the same (EK, QK) credential pair under any circumstances; OR generated by a non-CSPRNG source; and key size reductions below the active security profile minimum during operation.

| **KEY DOMAIN — THE EQUALITY PROHIBITION IS REMOVED (Rev 5.1)** Earlier revisions prohibited EK = QK. Rev 5.1 REMOVES that prohibition from the Canonical theorem-bearing profile, and implementations MUST NOT reject or resample an equal pair. Reason: Theorems 2 and 3 are proved over a uniform prior on the FULL product key space. Excluding the diagonal makes the key domain a proper subset that is not a submodule, and the First Isomorphism Theorem then no longer delivers constant fibres — the support result would likely survive, but the exact posterior-uniformity of Theorem 3 and the universality of the four-element coset of Theorem 2 are proved over the full module and are NOT proved over the punctured domain. Rejection sampling is precisely what punctures it. Restoring the full domain is what keeps both theorems literally true as stated. Algebraic analysis (unchanged): VKP is derived from eff_or and OKP as (VKP + DS_SEP) mod 16, so the two pointer paths remain distinct even when EK = QK; the cipher operates normally. Under independent uniform sampling EK = QK occurs with probability 16^−ⁿ = 2^−²⁵⁶ at HIGH. Should it occur, the only loss is the intended two-key architectural separation — the keystream for that one pair is a function of n key nibbles rather than 2n, which lowers the realized image for that pair while leaving every stated bound intact, and recovery of one master key would yield both. That is a 2^−²⁵⁶ event and no result in §8.8 is conditioned on avoiding it. Implementations SHOULD NOT return an error on equality; the former cipher-layer check is withdrawn. |
|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
All master keys (EK, QK) MUST be generated by a Cryptographically Secure Pseudo-Random Number Generator (CSPRNG) meeting the requirements of NIST SP 800-90A, SP 800-90B, or equivalent. EK and QK MUST be generated independently from separate CSPRNG invocations. THEOREM-BEARING PROFILE (normative): EK and QK MUST be sampled independently and UNIFORMLY over the full canonical key spaces — full entropy, 4n bits each (256 bits at HIGH). This is a hypothesis of Theorem 3, not a recommendation: a source supplying only partial min-entropy could in principle concentrate its support inside a single fibre of the keystream map, and the 2^128 posterior cannot be inferred from source min-entropy alone. Configurations supplying less than full entropy (for example 128 bits of min-entropy within a 256-bit key, the conformance floor of earlier revisions) constitute a SEPARATE profile requiring its own entropy analysis; no result of §8.8 applies to them. The OR MUST be generated per-record by a CSPRNG with entropy meeting the profile minimum.
...
Nonce uniqueness is PROBABILISTIC, supplied by the CSPRNG OR draw. The OR_EXP component contributes diffusion across all nonce positions and operational replay resistance; the deterministic-uniqueness guarantee claimed for it in earlier revisions is WITHDRAWN (§7.2). The eff_or is transmitted in the clear alongside the ciphertext.

OR_EXP self-referential call: OR_EXP is derived via PDAF Mode 1 self-application of OR_CTR_nibs (VK = OK = OR_CTR_nibs). The preimage structure of this call is distinct from the primary cipher path (where VK ≠ OK). Earlier revisions claimed a deterministic uniqueness guarantee resting on OR_CTR monotonicity. That claim is WITHDRAWN. It fails on two independent grounds: (1) eff_or combines the counter expansion with the CSPRNG OR by MOD16 addition, which is not injective in the counter — a different random draw can cancel a counter difference; and (2) the pointer derivation OR → VKP is itself non-injective (see the collision family below), so distinct eff_or values can still yield identical VKP and therefore an identical keystream under the same (EK, QK). OR_CTR is retained for diffusion and operational replay resistance. Nonce uniqueness is PROBABILISTIC, supplied by the CSPRNG draw. Pointer-derivation collision family (normative disclosure). The map OR → VKP = PDAF₁(OR,OR)\[:n\] is NOT injective. For a constant nonce OR = (k,k,…,k), every ValueKey lookup returns k regardless of which displacement the OffsetKey selects, so every output nibble is 2k mod 16. Hence VKP((k,…,k)) = VKP((k+8,…,k+8)) for every k in 0..7 — eight collision pairs at every profile. Because VKP determines VKC and OKC and hence W, colliding nonces yield an IDENTICAL keystream under the same (EK, QK): a two-time-pad condition. Exhaustive enumeration at small n shows the image of OR → VKP is a constant fraction of the nonce domain (approximately 33%, 39%, 16% and 29% at n = 2, 3, 4, 5), not an exponentially collapsing set. These small-scale measurements do NOT establish or bound the HIGH-profile pointer-state collision probability, and no such bound is claimed anywhere in this specification or in \[12\]; they indicate only that the image does not collapse exponentially at the sizes tested. What the family rules out is any structural injectivity or deterministic-uniqueness claim. Implementations MUST NOT substitute a low-entropy, constant, or counter-only nonce for the CSPRNG draw.

OR_EXP entropy scope: OR_EXP provides deterministic diffusion of OR_CTR across n key positions, not entropy expansion. The entropy of OR_EXP is bounded by the 64-bit OR_CTR: OR_EXP cannot contain more than 64 bits of entropy regardless of n. Note that distinct OR_EXP values do NOT imply distinct derivation state: eff_or mixes OR_EXP with the CSPRNG OR by MOD16 addition, which is not injective in the counter, and the downstream map OR → VKP is itself non-injective (§7.2). Uniqueness of eff_or is provided by the CSPRNG OR component and is probabilistic. The two components have distinct roles: deterministic counter state and diffusion (OR_CTR-based), and probabilistic nonce freshness (CSPRNG OR-based).
...
The domain separator DS_SEP = 0xF guarantees structural independence between the VKP and OKP derivation paths, preventing trivial relationships such as VKP = OKP. It is a public constant and contributes no entropy. Each record uses a freshly generated eff_or and freshly derives VKC and OKC, even if EK and QK remain unchanged across records. No structural uniqueness of the resulting pointer or derivation state is claimed: the map OR → VKP is non-injective (§7.2).
...
Phase 5 provides in-session key evolution that prevents reuse of W material across cycle boundaries within a session. An adversary who recovers the session keys (VKC, OKC) can advance the in-session key chain using VKP and OKP, which are deterministic functions of (EK, QK, eff_or). Fresh CSPRNG nonces make exact nonce reuse negligibly probable. No claim is made about derivation-state collision probability: the map OR → VKP is non-injective (§7.2) and no HIGH-profile bound on pointer-state collisions is proved anywhere in this specification or in \[12\]. Nonce freshness does not by itself isolate known plaintext across records under one continuing key state (§8.5); deployment isolation across records is provided by independent record credentials (§8.10), not by Phase 5 key update.

# 7. Key Management

## 7.1 Initial Credential Distribution

OpenID, EK, QK, and the initial OR_CTR value of zero are distributed out-of-band prior to first use. The initial key distribution channel must be assumed secure. Enqpy™ makes no claims about key establishment; it requires pre-shared secrets. EK and QK MUST be independently and uniformly generated. Equality is permitted and MUST NOT trigger rejection or resampling (§3.3).
...
## 7.2 Per-Record OR Generation, Counter Persistence, and Credential Retirement
...
**Enqpy requirement: NIL-Comm external-entropy rotation (NIL Method 2) is REQUIRED for Enqpy master-key rotation; the deterministic chain (NIL Method 1) is prohibited as the canonical Enqpy rotation mechanism.** This external-entropy rotation is the continuing-key-state rotation mechanism: it injects fresh secret material absent from the public transcript and so resets the known-plaintext accumulation of §8.5 for a continuing key state. The V5.1 deployment profile instead uses independent per-record credentials (§8.10): the continuing key state is bounded to a single record, and fresh per-record key material prevents accumulation across records. Coset propagation: both NIL methods preserve the four-element ciphertext-equivalent coset {EK, EK+8·1} × {QK, QK+8·1} — all four members produce the identical E_seed (Mode 0 shifts uniformly by +8 under a coset shift, and OWC cancels the +8 because each OWC output sums two nibbles of the same array) and the identical post-update credential pair. Ciphertext-only equivocation about the original credential therefore remains exactly 2 bits across NIL updates. The update maps all four coset members to the SAME new pair (a 4→1 collapse); under NIL Method 1 (deterministic chain), in the known-plaintext limit the post-rotation current-epoch key value is then determined (the chain is simulable). NIL Method 2’s external entropy (≥128-bit, out-of-band, unknown to the adversary) re-injects key uncertainty at each rotation, which is why Enqpy requires it. (Formal sufficiency of this reset against the coset/linearity structure would require a separately proved reset property; the continuing-key-state path is out of scope for the V5.1 deployment claim.)
...
| Quantum-Era Posture (ciphertext-only)                                                | The underdetermination property is platform-independent; within the ciphertext-only model a quantum computer cannot erase or reduce the proved algebraic support bound. Algebraically stated in \[12\], Corollary 2. Not a deployment-grade quantum-security guarantee; see §8.5.                                                                                                                                                                                                                                                                                                                                                                           |
...
| In-Session Key Evolution                                                             | Working keys for each cycle are derived from the prior session key state via Phase 5 PDAF Mode 1 update. This prevents reuse of W material across cycle boundaries within a session. Fresh CSPRNG nonces make exact nonce reuse negligibly probable; derivation-state collisions are possible because OR → VKP is non-injective and no HIGH-profile collision bound is claimed (§7.2). Nonce freshness does **not** by itself provide known-plaintext isolation across records under one continuing key state (§8.5). Deployment isolation across records is provided by independent record credentials (§8.10), not by Phase 5 update or nonce uniqueness. |
| Plaintext Equivocation (Enqpy, ciphertext-only) — **primary confidentiality result** | Enqpy™ (Case-1 W generation) achieves │S(CT,OR)│ ≥ 2^128 (HIGH profile, n=64) under ciphertext-only observation: for any ciphertext and public nonce, at least 2^128 plaintexts are consistent with (CT,OR) — a support bound that holds for every plaintext distribution. Under the stated uniform key and plaintext priors the posterior is uniform over the full consistent set, so H(PT│CT,OR) = H∞(PT│CT,OR) ≥ 128 bits (closed). Independent of key equivocation. Proved in \[12\], Theorem 3. Immune to equivalent-key normalization.                                                                                                                |
...
| RIGHT — Forward Propagation | Break future sessions from a broken session.     | Fresh CSPRNG nonces make exact nonce reuse negligibly probable; pointer/derivation-state collisions remain possible because OR → VKP is non-injective (§7.2), and no HIGH-profile collision bound is claimed. Nonce freshness does **not** by itself isolate known plaintext across records under one continuing key state (§8.5); forward isolation in deployment is provided by independent record credentials (§8.10), not by Phase 5 update or nonce uniqueness alone.                                                                                                                                                                                                                                                                                               |
| LEFT — Backward Propagation | Break past sessions from a broken session.       | Fresh CSPRNG nonces make exact nonce reuse negligibly probable. Pointer/derivation-state collisions are possible because the nonce-derivation map OR → VKP is non-injective (§7.2); no HIGH-profile collision bound is claimed. Cross-record isolation comes from independent record credentials (§8.10), not from nonce uniqueness and not from any one-wayness of PDAF. PDAF Mode 1 is a many-to-one MOD16 mixing map, not a one-way function in the cryptographic sense, and no backward-propagation claim rests on it.                                                                                                                                                                                                                                               |
| DOWN — Partial KP Attack    | Recover unknown PT from partial known-plaintext. | Enqpy: within a window, a rank-complete set of known W values (e.g. 64 known bytes at HIGH) linearly determines the remaining window W. This does NOT directly read out the session keys VKC/OKC or the master keys (the many-to-one PDAF Mode 1 mixing still leaves ≥ 2 (worst case) / ≥ 16 (non-degenerate) candidate keys per compatible OffsetKey). However, the exposure is **not** confined by fresh eff_or under one continuing key state: it accumulates, and the published reference instance demonstrates that two fully-known windows can suffice to enable cross-OR decryption — the general threshold is uncharacterized (§8.5). Deployment confinement is provided by independent record credentials (§8.10), which bound any exposure to a single record. |
...
**Status of these figures.** The 2¹² and 32-key residuals are MEASURED ON THE PUBLISHED REFERENCE INSTANCE — computed from the public Case-1 W-generation map and a recorded (OR₁, OR₂) pair — and are not proved for every admissible nonce pair. No universal rank theorem covering all distinct (OR₁, OR₂) is claimed. What the reference instance establishes is that TWO FULLY-KNOWN WINDOWS UNDER ONE CONTINUING KEY STATE CAN BE SUFFICIENT to enable cross-OR decryption. It does NOT establish that approximately two are generally sufficient; the number required for arbitrary nonce pairs is not yet characterized. Deployment guidance is drawn from the demonstrated sufficiency — a continuing key state must not be allowed to span material an adversary could fully know — not from an assumed average. The exact residual counts are instance measurements pending either a universal rank theorem or a reported distribution across many pairs.

- **Partial known plaintext** raises the threshold (more windows are required as the known fraction per window falls).

A fresh nonce per record does **not** contain this accumulation; intervening unknown records and the deterministic Phase 5 update do **not** reset it (both are simulable by the adversary). The mechanism that prevents accumulation across records is **per-record credential separation** (§8.10): fresh per-record key material under either conforming key-supply profile injects secret material absent from the public transcript and confines any exposure to its own record. Within a record the deterministic Phase 5 chain links the internal windows, so known plaintext can extend across the record; smaller records bound this exposure more tightly.
...
| AES-256-GCM       | Computational hardness (block cipher)                                                       | Partial (Grover halves)                             | not comparable (see note)                                                 |
| ChaCha20-Poly1305 | Computational hardness (ARX)                                                                | Partial (Grover halves)                             | not comparable (see note)                                                 |
...
**Note on the equivocation column.** Enqpy’s figure is a CIPHERTEXT-ONLY key-equivocation floor. The corresponding quantity for AES-256-GCM and ChaCha20-Poly1305 under ciphertext-only observation is not zero and is not known to be bounded — a conventional cipher’s key is not information-theoretically determined by ciphertext alone. The value approaches zero only under sufficient KNOWN PLAINTEXT, past the unicity distance. Earlier revisions printed “0 bits” in this column, which compared a known-plaintext quantity with a ciphertext-only one; that entry is withdrawn. The columns to its left remain the meaningful comparison: what the security rests on, and whether that basis is a proof or an assumption.

Deployment and performance:

| **Cipher**        | **Key size**     | **Auth**                                                                          | **Speed**                                                                                                                                             |
|-------------------|------------------|-----------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| Enqpy™ PDAF_SEC   | ≥ 128–256 bit    | encrypt-then-MAC required (Poly1305/HMAC); independent record credentials (§8.10) | ~900 MB/s @2KB with Poly1305 authentication (like-for-like with the rows above); ~870 MB/s cipher-only, scaling to ~2,300 @≥256KB cipher-only (§10.2) |
...
**Primary result — Plaintext Equivocation (Theorem 3 in \[12\]).** For Enqpy (Case-1 W generation, Canonical Configuration), under ciphertext-only observation at least 2^128 plaintexts are consistent with any observation — \|S(CT,OR)\| ≥ 2^128 for HIGH profile (n=64) — a support bound that holds for every plaintext distribution. Under the stated uniform key and plaintext priors the posterior is uniform over the full consistent set, and Shannon entropy and min-entropy coincide: H(PT\|CT,OR) = H∞(PT\|CT,OR) = log₂\|S(CT,OR)\| ≥ 128 bits, a closed message-axis result. This is the cipher’s confidentiality claim. It is independent of equivalent-key normalization: the consistent plaintext set is a property of the keystream output, not of how keys are counted.

**On the tightness of the 2^128 floor.** The bound is proved by a deliberately loose argument and is conservative at HIGH. It is derived by maximising two-cycles in the functional graph of the displacement map, which bounds the loss by 16^(n/2). That configuration is not reachable at HIGH: since VKP\[p\] ∈ ℤ₁₆, the displacement satisfies σ(p) − p ∈ \[1,16\], and around a cycle of length k the displacements sum to a multiple of n, so k ≥ ⌈n/16⌉. The minimum admissible cycle length is therefore 2 at LOW (n=32), 3 at MEDIUM (n=48) and 4 at HIGH (n=64) — the all-two-cycle worst case exists only at LOW. Applying the same parity accounting over cycles of length ≥ 4 gives ≥ 2^192 at HIGH, and direct computation of the joint map image by Smith normal form over ℤ₁₆ places it higher still (smallest image found over admissible VKP: 2^440; random nonce: approximately 2^504, against the 8n = 512-bit finite-key ceiling). The 2^128 figure is retained as the stated claim because it is PROVED rather than measured; tightening it to a proved minimum over admissible VKP is open work. IMPLICATION FOR PROFILE SELECTION: at LOW the bound is tight and the floor is correspondingly 2^64. Deployments requiring the theorem-bearing guarantee SHOULD use HIGH.

**Supporting result — Key-Axis Keystream-Equivalence Coset (Theorem 2 in \[12\]).** The four-element coset {EK, EK+8·1} × {QK, QK+8·1} is a permanent keystream-equivalence class: all four pairs produce identical (VKC,OKC), identical keystream, and identical ciphertext for every nonce and plaintext. This gives a non-vanishing **ciphertext-only key-equivocation floor of ≥ 2 bits** for every transcript. KEY DOMAIN NOTE (Rev 5.1): the four-element coset is universal over the full product key space, and Rev 5.1 removes the former EK ≠ QK prohibition (§3.3) so that the implemented key domain is exactly that space. The universal ≥ 2-bit statement therefore holds unqualified, with no exceptional set. The exact four-key residual ambiguity, H(EK,QK) = log₂(4) = 2 bits, holds only under displacement-complete **effective-keystream** observation (a known-plaintext-strength condition, not ciphertext-only); under ciphertext-only observation the key equivocation is at least 2 bits and generally larger.

**What Theorem 3 adds beyond a conventional stream cipher.** The sharpest objection to this result is not that it is wrong but that it is not new, and it deserves a direct answer. THE OBJECTION: for any additive stream cipher CT = PT ⊕ W. If PT is uniform and independent of W, then CT is uniform and independent of W (hence of the key), and the plaintext posterior is the XOR translate of the keystream distribution: Pr\[PT = p \| CT = c\] = Pr\[W = p ⊕ c\]. The residual plaintext uncertainty is therefore exactly the keystream uncertainty, H(PT \| CT) = H(W), and the consistent plaintext set is c ⊕ supp(W). (PT and CT are NOT in general statistically independent — that would require W to be uniform over the whole message space, which no finite-key cipher achieves. The posterior identity above is the correct statement and is what the argument needs.) This holds for AES-256-CTR and ChaCha20 exactly as it holds for Enqpy. Counted naively, a 2,048-byte AES-256-CTR ciphertext is consistent with roughly 2^256 plaintexts — a LARGER set than the 2^128 floor proved here. On the count alone, Enqpy does not do better.

**What is conceded.** All of it. Ciphertext-only ambiguity under a uniform message prior is not a phenomenon this construction created, and the floor proved here is not larger than the naive key-space count for a conventional 256-bit cipher. Neither construction’s uniform-posterior conclusion extends to a redundant source. Any framing of this result as “more secrecy than AES” is unsupported by anything in this document or in \[12\].

**What differs.** Not the size of the consistent set but its epistemic status. For a conventional cipher no lower bound on that set is proved: the 2^256 figure is a heuristic count over the key space, and confidence in it rests on the assumption that no analytic structure collapses it. That assumption is supported by sustained public cryptanalysis — the right basis for deploying AES — but it is an assumption, and a structural advance could falsify it. Nothing has been PROVED about the structure of a conventional cipher’s consistent plaintext set. For Enqpy that set is exhibited rather than assumed. Theorem 3 establishes three properties of it by computation over ℤ₁₆, with no hardness assumption at any step: (i) STRUCTURE — it is the XOR translate of the image of a ℤ₁₆-module homomorphism, an object whose shape is known rather than conjectured; (ii) UNIFORMITY — the fibres are provably constant, so under the uniform key and plaintext priors of Theorem 3 the posterior is exactly uniform over the whole consistent set (ε = 0, not a negligible bound), and under those priors no consistent plaintext is more likely than any other; (iii) A PROVED FLOOR — the size bound holds for every ciphertext and every nonce, not generically or on average.

**The contribution, stated exactly.** Enqpy proves an exact support, fibre-uniformity and lower-bound statement about its ciphertext-only ambiguity that forms no part of the standard computational security argument for a conventional cipher. This is deliberately narrower than saying Enqpy “proves what conventional ciphers assume”: AES and ChaCha20 do not assume this statement, they target a different notion (computational PRP/PRF and IND-style security) for which it is neither claimed nor needed. The claim is not that Enqpy’s ciphertext-only ambiguity is larger; it is that this particular property is a theorem rather than an unexamined by-product. A theorem about counting cannot be eroded by an algorithmic advance or by a quantum computer, because it was never protected by the difficulty of a computation in the first place. That is the entire claim, and it is narrower than the phrase “information-theoretic security” is usually taken to mean.

**What it does not give.** It is not Perfect Secrecy — the finite-key ceiling is 8n bits per key epoch. It is not known-plaintext security — the boundary is stated and quantified in §8.5. It is not IND-CPA or IND-CCA security, and no such claim is made anywhere. It is not keystream indistinguishability — the Case-1 keystream is deliberately linear and trivially distinguishable. And once the plaintext source is not uniform it does not give the uniform posterior or the min-entropy equality — those are stated under the uniform plaintext model of this section. The support bound is unaffected and holds for every source. How concentrated a source-conditioned posterior becomes, and whether the classical unicity estimate — an expectation over a random-cipher model — transfers to a consistent set with the algebraic structure established above, are not characterized here.

**The price.** The two halves of this construction are the same fact seen twice. The linearity that makes Theorem 3 provable is the linearity that makes the §8.5 known-plaintext boundary unavoidable: a keystream map whose ciphertext-only ambiguity can be counted exactly is a module homomorphism, and a module homomorphism is invertible from enough known plaintext. Within THIS construction the two are the same property and neither can be removed without the other. We do not claim the stronger and unproved statement that no other construction could combine a provable support theorem with known-plaintext resistance; that question is open, and the falsified attempts recorded elsewhere in this document are evidence that it is hard, not that it is impossible. Whether that trade is worth making — proved ciphertext-only ambiguity, purchased with no known-plaintext resistance in the bare core — is a deployment judgement rather than a mathematical one, and neither this document nor \[12\] makes it for the reader.
...
- Message axis: \|S(CT,OR)\| ≥ 2^128 for every plaintext distribution; under the stated uniform key and plaintext priors the posterior is uniform, so H(PT\|CT,OR) = H∞(PT\|CT,OR) ≥ 128 bits (closed; ciphertext-only; Theorem 3).
- Key axis: ciphertext-only key-equivocation floor H(EK,QK \| T^CO) ≥ log₂(4) = 2 bits for every transcript; exact four-key tightness H(EK,QK) = 2 bits under effective-keystream observation (Theorem 2). Conditional independence T\_{\>t} ⊥ (EK,QK) \| T\_{≤t} holds within the coset.
...
| **Document**                                                                                       | **Content**                                                                             |
|----------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------|
| \[12\] Enqpy: Ciphertext-Only Plaintext Equivocation in a Finite-Key MOD16 Stream Cipher (Rev 5.1) | Lemma 1 (MOD16 fiber size = 16, algebraic); Theorem 1 (PDAF Mode 1 preimage lower bound |
...
### R4 — Key Domain (former EK ≠ QK Requirement — WITHDRAWN)

R4 IS WITHDRAWN IN REV 5.1. The former EK ≠ QK requirement is removed from the Canonical theorem-bearing profile (§3.3): it punctured the key domain over which Theorems 2 and 3 are proved. Setting EK = QK does not reduce the algebraic four-element coset in the full product space — it reduces the intended two-key architectural separation, and nothing else. Implementations MUST NOT reject an equal pair. The Canonical Configuration otherwise preserves the two-key architecture: EK protects the VKC derivation path; QK protects the OKC derivation path. Setting EK = QK collapses the two-key separation and reduces the two-key architectural separation (it does not reduce the algebraic four-element coset in the full product space).
...
The formal results of §8.8 are stated for the ciphertext-only, single-key-epoch model. The known-plaintext boundary of §8.5 shows that a continuing key state is what permits known-plaintext accumulation. The published reference instance demonstrates that two fully-known windows under one continuing key state can suffice for cross-OR decryption; the threshold for arbitrary nonce pairs is not yet characterized. The deployment profile in this revision confines that condition to a single **record**: a payload is divided into independently keyed records, **each record is encrypted under its own fresh credential material**, and a record is processed internally as one or more 2,048-byte Enqpy windows chained by Phase 5. Every record is authenticated by encrypt-then-MAC (§8.6). The normative rules and proof basis are given in §8.10.1.
...
*A continuing-key-state external-entropy rotation path (one credential spanning multiple records, reset by fresh NIL-Comm Method 2 entropy) is possible but is out of scope for this revision: it would require a separately proved reset property and is not part of the V5.1 deployment claim.*

## 8.10.1 Record Deployment and Internal Windowing

§8.5 establishes two distinct properties, which require different treatment. The first is *within-record determinability*: a rank-complete set of known plaintext within a record determines the remaining keystream of **that record** — at HIGH, 64 known bytes determine an internal 2,048-byte window, and on the published reference instance two fully-known internal windows under the record’s key sufficed to collapse the record’s forward key diversity, the general threshold being uncharacterized (§8.5). The second is *cross-record propagation*: with independent per-record key material, that determination does **not** carry into other records.
...
**Record credential rule (normative).** A given (EK, QK) credential MUST encrypt at most one record. The record boundary is the credential boundary: a new record MUST begin a fresh, non-reused credential. (INDEPENDENT is reserved in this specification for the statistical-independence claim of the information-theoretic key-supply profile; it is not a synonym for separately supplied.) Two profiles are conforming, differing only in record size, corresponding exactly to the two profiles of the deployment corollary in \[12\]. (a) SINGLE-WINDOW RECORDS (maximally tight): each record is at most one 2,048-byte Enqpy window at HIGH, confining any known-plaintext exposure to a single window. Deployments handling predictable or partially-known plaintext SHOULD use this profile. (b) MULTI-WINDOW RECORDS: a record comprises several windows chained by the Phase 5 cross-combined update (with \[:n\] truncation) under one per-record credential. The ciphertext-only results extend to profile (b) by the multi-window composition lemma of \[12\] — the joint map across k windows remains a ℤ₁₆-module homomorphism, the session floor inherits the single-window floor, and the 8n-bit ceiling applies. Known plaintext, however, is NOT confined within a multi-window record: the Phase 5 chain is public and simulable, so a rank-complete known-plaintext set anywhere in the record determines the remainder of that record (§8.5). Record size is therefore the security/throughput dial, and the maximal per-record exposure grows with it.

**Record segmentation (normative).** A payload is divided at the application layer into records. Each record is encrypted under its own per-record credential material (EK_r, QK_r) (§7.1 CSPRNG requirements; no EK_r ≠ QK_r restriction — see §3.3), its own per-record nonce eff_or_r (§7.2), and its own encrypt-then-MAC tag over eff_or_r ‖ CT_r ‖ record metadata (§8.6), together with ordering and length metadata sufficient for reassembly. Two conforming key-supply profiles are defined:
...
## 8.11 Optional Known-Plaintext Hardening (ESE)

The known-plaintext boundary of §8.5 is structural and is confined operationally by independent record credentials (§8.10). For deployments where known plaintext is an explicit part of the threat model and record-sizing confinement alone is insufficient, this revision specifies an **optional** within-record hardening layer, **ESE** (Enqpy-Scramble-Enqpy). ESE raises the within-record known-plaintext threshold; it is **not** part of the proved cipher core, it is **not** required for any result in §8.8, and the known-plaintext security it adds belongs to the hardening layer and is **computational**. The ciphertext-only results of §8.8 and \[12\] are unchanged by its presence: the base cipher operation is untouched and each record remains the single-key-epoch object those theorems cover. ESE is experimental.

**Construction.** A hardened record is the sandwich `C = S(P + W₁) + W₂`, where `+` is per-nibble mod-16 addition over ℤ₁₆. W₁ and W₂ are two **independent** per-record Enqpy keystreams (PDAF_SEC under domain-separated per-record credentials); S is a fresh, secret, per-record byte-permutation of the window positions, generated only by an approved PRF/KDF/CSPRNG permutation construction (never a low-entropy or ℤ₁₆-affine map). Base Enqpy’s record encryption (Phase 4, §6.3) is byte XOR; ESE’s two masking steps instead use mod-16 addition — the ℤ₁₆ structure on which the obstruction analysis rests — and ESE composes Enqpy keystreams around a secret permutation without altering the base cipher. Concretely: plaintext bytes are read as two nibbles; W₁ and W₂ are applied nibblewise; S permutes byte positions, carrying both nibbles of each byte together; ciphertext is re-packed as bytes. Decryption runs the sandwich in reverse. The canonical S is a keyed butterfly (Benes-class) conditional-exchange network, which is bit-identical in software and silicon.

**Conditional guarantee (PKP obstruction).** Against base Enqpy the known-plaintext attack runs on W = PT ⊕ CT (byte XOR): a known plaintext value at a known position yields a known keystream value at a known position, which is rank-complete (§8.5). ESE breaks that chain conditionally on S remaining secret, at three stated levels of strength:

- **Structural obstruction.** With S secret, partial known plaintext does not directly reveal a rank-complete positioned set for either Enqpy layer; the adversary observes only C and cannot map a known input position to the keystream coordinate that position constrains.

- **Structural + measured.** Even if S were known, the combined system is ℤ₁₆-linear in 4n unknowns with rank 4n−4 — the known-plaintext threshold roughly doubles, to ≈126–266 known bytes against ≈64 for base Enqpy at HIGH.

- **Empirical.** With S fresh, secret, and from the approved construction, no tested single-record attack recovers S, W₁, W₂, or the plaintext materially faster than attacking the permutation directly.

The guarantee is **conditional** — it raises a recovery threshold and is not an unconditional or information-theoretic known-plaintext-security claim — and **computational**, resting on the secret per-record permutation. The formal treatment is the companion Conditional PKP Obstruction Lemma \[13\]; the quantitative n=64 secret-S threshold remains an **open measurement item**, not an analytically closed bound.

**Instantiations.** Two tested reference instantiations of S are provided: an arithmetic position-Feistel for software (≈320 MB/s scalar, about 11% slower than ChaCha20-Poly1305) and the keyed butterfly/Benes network for silicon (multi-GB/s class). Both cleared the documented falsification suite. Reference implementations exist in C and VHDL and agree on a shared interoperability test vector \[14\].

| **NORMATIVE ESE DEPLOYMENT CONDITIONS (this profile only)** ESE is used only with all of the following. (1) **No reuse.** S, W₁, W₂, and the network switch-bit material MUST be fresh per record; reuse of the triple across records exposes S from a few records and is catastrophic. (2) **Independent credentials.** W₁, W₂, and the switch-bit keystream MUST be derived from domain-separated, independent per-record credentials. (3) **ESE record = one window.** Each hardened record is one 2,048-byte Enqpy window at HIGH; this sizing is specific to ESE and does not replace the general record-credential rule of §8.10.1. (4) **Authenticate.** Each hardened record MUST be authenticated by the host protocol’s encrypt-then-MAC / AEAD construction over metadata, nonce fields, and ciphertext (§8.6). (5) **Approved generator.** S MUST be produced only by an approved PRF/KDF/CSPRNG permutation construction. Public-register statement: ESE adds known-plaintext protection that belongs to the wrapper; it is not a claim that base Enqpy is known-plaintext secure. |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
- **Nil-Communication Key Update.** Deterministic NIL Method 1 supports master-key evolution without data exchange; NIL Method 2 adds external-entropy rotation, requiring out-of-band fresh entropy (E_ext) and synchronization. Neither method places a key exchange on the ciphertext channel. NIL Method 1 provides no forward secrecy; NIL Method 2 provides forward secrecy and is the continuing-key-state rotation mechanism (out of scope for the V5.1 deployment profile, which uses independent record credentials, §8.10). See Section 7.4.

- **PDAF-Expanded Nonce Diffusion.** A 64-bit monotonic counter is expanded to n nibbles via PDAF Mode 1 before mixing with the CSPRNG-generated OR, providing strong diffusion across all nonce positions and operational replay resistance. Nonce uniqueness is probabilistic (CSPRNG); the earlier deterministic-uniqueness claim is withdrawn (§7.2).
...
- **Optional Known-Plaintext Hardening (ESE).** An optional, experimental wrapper — `C = S(P + W₁) + W₂` with a fresh secret per-record permutation S between two independent Enqpy keystream masks — raises the within-record known-plaintext threshold for deployments that need it. The guarantee is conditional and computational and belongs to the wrapper; the proved cipher core and §8.8 are unchanged. See §8.11; formal treatment in \[13\].
...
Key equality (Rev 5.1): the former EK ≠ QK enforcement is WITHDRAWN. PDAF_SEC implementations MUST NOT return an error when EK and QK are byte-identical, and callers MUST NOT resample on equality. Rejection sampling punctures the key domain over which Theorems 2 and 3 are proved (§3.3). Keys are simply sampled independently and uniformly; equality arises with probability 2^−²⁵⁶ at HIGH and is operationally harmless.
...
*Note: OR_CTR values 1 and 2 produce low-entropy OR_EXP output due to leading zeros in the 16-nibble representation. This is expected and correct — OR_EXP contributes counter state and diffusion, not uniqueness; the deterministic-uniqueness claim is WITHDRAWN (§7.2). Statistical uniqueness of eff_or is provided by the CSPRNG OR component mixed in Phase 1 (eff_or\[i\] = MOD16\[or_nibs\[i\]\]\[OR_EXP\[i\]\]). OR_CTR 0xA5C3 shows the propagation behaviour for a non-trivial value.*
...
**Relationship to the security claims.** Enqpy’s guarantees are information-theoretic equivocation and min-entropy statements (Theorems 2 and 3), not claims of keystream indistinguishability, and no such claim is made anywhere in this document. A known-plaintext adversary is expected to recover W, including its structure; both theorems are stated against the complete transcript T^∞ and already condition on this recovery. On the KEY axis the \[+8\] coset is unconditional: those four credential pairs produce an identical keystream for every nonce and every plaintext, so no observation of any kind separates them. The ≥ 2-bit key-equivocation floor of Theorem 2 therefore survives arbitrary known-plaintext keystream recovery, and a demonstrated keystream recovery is, formally, a finite subset of T^∞ that cannot do better than the proof already grants to an adversary holding the whole of it. The MESSAGE axis must NOT be argued the same way. Theorem 3 conditions on (CT, OR) only, and does not extend to conditioning that additionally includes known plaintext, since further conditioning reduces uncertainty. Any claim that known-plaintext keystream recovery cannot reduce plaintext min-entropy below 128 bits is WITHDRAWN: it would contradict §8.5, which states that a rank-complete known-plaintext set determines the remaining keystream of the window. Known plaintext is governed by §8.5 and confined by the record credential rule of §8.10 — not by Theorem 3. The visible keystream structure is the mechanism of the ciphertext-only guarantee, not a counterexample to it. The companion note “Recovering the Keystream Is Not Breaking the Cipher” develops this argument in full and in plain language.
...
\[12\] McGough, P. / NQP LLC (2026). Enqpy™: Ciphertext-Only Plaintext Equivocation in a Finite-Key MOD16 Stream Cipher, with a Characterized Known-Plaintext Boundary, Revision 5.1. Public record: enqpy.com and github.com/nqp-llc/enqpy, 2026. Establishes Lemma 1 (MOD16 fiber size), Theorem 1 (PDAF Mode 1 preimage lower bound \|P(O\*)\| ≥ 2 worst case / ≥ 16 non-degenerate), Theorem 3 (Ciphertext-Only Plaintext Equivocation — primary result: \|S(CT,OR)\| ≥ 2^128 for HIGH profile for every plaintext distribution; under the stated uniform key and plaintext priors, posterior uniform over the full consistent set and H = H∞ ≥ 128 bits) and Theorem 2 (the permanent four-key \[+8\] keystream-equivalence coset — a ciphertext-only key-equivocation floor of ≥ 2 bits, exact only under effective-keystream observation; conditional independence T\_{\>t} ⊥ (EK, QK) \| T\_{≤t}), the §16 known-plaintext boundary (cross-OR transfer demonstrated on the published reference instance to be possible with two fully-known windows under a continuing key state; the general threshold is uncharacterized), and Proposition 2 (information-theoretic cross-record non-accumulation under independently sampled per-record keys), with Corollary 2 (quantum-era posture — bounds are algebraic, no public-key hardness assumption). Appendix B provides the extended algebraic proofs: exact preimage count formula \|P(O\*)\| = Σ\_{compatible OK} ∏\_i f(μ\_{i,odd}(OK)) \[f(0)=16, f(k≥1)=2\]; equivocation tightness (rank argument over ℤ₁₆); cycle case completeness (ℤ₁₆ vs. ℤ₂); exact solution count with cycles (induction proof). All ciphertext-only bounds are algebraic and rest on no computational-hardness assumption.

\[13\] McGough, P. / NQP LLC (2026). Enqpy™ ESE — Conditional PKP Obstruction Lemma, Rev 5.0. Companion proof-register note for the optional known-plaintext hardening layer (§8.11): structural obstruction under secret S, the known-S combined-system rank 4n−4 with a ≈126–266-byte known-plaintext threshold (against ≈64 for base Enqpy at HIGH), and the empirical secret-S equivocation evidence (falsification suite; n=4/n=64 determinacy sweeps). The n=64 quantitative secret-S threshold is stated as an open measurement item. The result is conditional and computational and does not modify the base-cipher results of \[12\].

\[14\] NQP LLC (2026). Enqpy™ Known-Plaintext Hardening — Implementation Extension, with C and VHDL reference implementations and a shared interoperability test vector. Specifies the ESE construction `C = S(P + W₁) + W₂`, the two tested permutation instantiations (software arithmetic-Feistel; silicon keyed butterfly/Benes network), and the normative deployment conditions of §8.11.

---

Enqpy™ Formal Cryptographic Description — Revision 5.1 · © 2026 NQP LLC · Licensed under CC-BY-4.0 · www.enqpy.com
